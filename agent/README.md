...
该 Node 进程领取排队中的 `agent_runs`，为每个任务创建一个能力受限的 Pi `AgentSession`，并保存待人工审核的结果。当前支持官方档案补全、参考调研、笔记及素材采集、选题提炼、Brief 生成和草稿生成六类任务。Worker 独立续租任务；最终状态只能由当前持有执行权的 Worker 写入。

公共运行约束在 `RUNTIME.md`，任务专用约束在 `runtimes/`。

1. 按顺序应用项目数据库迁移；Agent 接入包含 `supabase/migrations/202609080001_agent_execution.sql`、`supabase/migrations/202609080002_generalize_agent_runs.sql` 和后续执行层迁移。
