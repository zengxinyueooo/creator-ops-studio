...
export function AnalyticsPage() {
  return (
    <>
      ...
      <section className="analytics-grid"><div className="panel chart-panel"><div className="panel-heading"><div><h2>内容表现趋势</h2><p>按发布内容的综合互动分</p></div><BarChart3 size={19} /></div><div className="bar-chart">{bars.map((height, index) => <div className="bar-wrap" key={index}><div className="bar" style={{ height: `${height}%` }} /><span>{index + 1}日</span></div>)}</div></div><div className="panel ranking-panel"><div className="panel-heading"><div><h2>高表现栏目</h2><p>按平均互动排序</p></div><ArrowUpRight size={18} /></div>{[['高能片段', '92'], ['角色观察', '86'], ['新番追更', '81'], ['完结安利', '74']].map(([name, score], index) => <div className="ranking-row" key={name}><b>0{index + 1}</b><span>{name}</span><strong>{score}</strong></div>)}</div></section>
    </>
  )
}
