...
11. 人工编辑；用户明确触发 `xiaohongshu-draft-staging` 后可暂存到创作者中心草稿箱，再由用户复核并手动发布。

不得跳过前置条件，不得将生成的候选自行改为人工审核通过状态。不得通过 Agent 工作流在小红书正式发布、点赞、关注、收藏或评论。草稿暂存仅可由用户明确触发，并遵守 `xiaohongshu-draft-staging` 的停止边界。
...
完成本项目的修改及验证后，将本次改动提交并推送到 GitHub；提交前核对未跟踪文件，不覆盖用户的其他工作。
